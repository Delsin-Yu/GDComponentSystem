...
[Tool, GlobalClass]
public partial class CodeEditComponentHost : CodeEdit, IComponentHost
{
    ...
    private static extern void OnComponentAdded(NodeComponent nodeComponent, CodeEditComponentHost hostInstance);

    [UnsafeAccessor(UnsafeAccessorKind.Method, Name = "_OnComponentRemoved")]
    private static extern void OnComponentRemoved(NodeComponent nodeComponent, CodeEditComponentHost hostInstance);
    ...
    public override void _ConfirmCodeCompletion(bool @replace)
    {
        ...
        {
            if (component is not CodeEditComponent typedComponent) continue;
            typedComponent._ConfirmCodeCompletion(@replace);
        }
    }
    ...
    public override Array<Dictionary> _FilterCodeCompletionCandidates(Array<Dictionary> @candidates)
    {
        ...
        {
            if (component is not CodeEditComponent typedComponent) continue;
            return typedComponent._FilterCodeCompletionCandidates(@candidates);
        }
    ...
    public override void _RequestCodeCompletion(bool @force)
    {
        ...
        {
            if (component is not CodeEditComponent typedComponent) continue;
            typedComponent._RequestCodeCompletion(@force);
        }
    }
    ...
    public override void _Backspace(int @caretIndex)
    {
        foreach (var component in _components)
        {
            if (component is not TextEditComponent typedComponent) continue;
            typedComponent._Backspace(@caretIndex);
        }
    }
    ...
    public override void _Copy(int @caretIndex)
    {
        ...
        {
            if (component is not TextEditComponent typedComponent) continue;
            typedComponent._Copy(@caretIndex);
        }
    }
    ...
    public override void _Cut(int @caretIndex)
    {
        ...
        {
            if (component is not TextEditComponent typedComponent) continue;
            typedComponent._Cut(@caretIndex);
        }
    }
    ...
    public override void _HandleUnicodeInput(int @unicodeChar, int @caretIndex)
    {
        ...
        {
            if (component is not TextEditComponent typedComponent) continue;
            typedComponent._HandleUnicodeInput(@unicodeChar, @caretIndex);
        }
    }
    ...
    public override void _Paste(int @caretIndex)
    {
        ...
        {
            if (component is not TextEditComponent typedComponent) continue;
            typedComponent._Paste(@caretIndex);
        }
    }

    /// <inheritdoc />
    public override void _PastePrimaryClipboard(int @caretIndex)
    {
        foreach (var component in _components)
        {
            if (component is not TextEditComponent typedComponent) continue;
            typedComponent._PastePrimaryClipboard(@caretIndex);
        }
    }
