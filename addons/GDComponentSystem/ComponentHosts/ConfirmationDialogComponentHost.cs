...
[Tool, GlobalClass]
public partial class ConfirmationDialogComponentHost : ConfirmationDialog, IComponentHost
{
    ...
    private static extern void OnComponentAdded(NodeComponent nodeComponent, ConfirmationDialogComponentHost hostInstance);

    [UnsafeAccessor(UnsafeAccessorKind.Method, Name = "_OnComponentRemoved")]
    private static extern void OnComponentRemoved(NodeComponent nodeComponent, ConfirmationDialogComponentHost hostInstance);

    /// <inheritdoc />
    public override Vector2 _GetContentsMinimumSize()
    {
        foreach (var component in _components)
        {
            if (component is not WindowComponent typedComponent) continue;
            return typedComponent._GetContentsMinimumSize();
        }
        return base._GetContentsMinimumSize();
    }
