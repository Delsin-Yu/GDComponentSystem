...
[Tool, GlobalClass]
public partial class LimitAngularVelocityModifier3DComponentHost : LimitAngularVelocityModifier3D, IComponentHost
{
    ...
    private static extern void OnComponentAdded(NodeComponent nodeComponent, LimitAngularVelocityModifier3DComponentHost hostInstance);

    [UnsafeAccessor(UnsafeAccessorKind.Method, Name = "_OnComponentRemoved")]
    private static extern void OnComponentRemoved(NodeComponent nodeComponent, LimitAngularVelocityModifier3DComponentHost hostInstance);
    ...
    public override void _ProcessModification()
    {
        ...
        {
            if (component is not SkeletonModifier3DComponent typedComponent) continue;
            typedComponent._ProcessModification();
        }
    }
    ...
    public override void _ProcessModificationWithDelta(double @delta)
    {
        ...
        {
            if (component is not SkeletonModifier3DComponent typedComponent) continue;
            typedComponent._ProcessModificationWithDelta(@delta);
        }
    }
    ...
    public override void _SkeletonChanged(Skeleton3D @oldSkeleton, Skeleton3D @newSkeleton)
    {
        ...
        {
            if (component is not SkeletonModifier3DComponent typedComponent) continue;
            typedComponent._SkeletonChanged(@oldSkeleton, @newSkeleton);
        }
    }

    /// <inheritdoc />
    public override void _ValidateBoneNames()
    {
        foreach (var component in _components)
        {
            if (component is not SkeletonModifier3DComponent typedComponent) continue;
            typedComponent._ValidateBoneNames();
        }
    }
