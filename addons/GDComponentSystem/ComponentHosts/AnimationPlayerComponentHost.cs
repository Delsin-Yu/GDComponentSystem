...
[Tool, GlobalClass]
public partial class AnimationPlayerComponentHost : AnimationPlayer, IComponentHost
{
    ...
    private static extern void OnComponentAdded(NodeComponent nodeComponent, AnimationPlayerComponentHost hostInstance);

    [UnsafeAccessor(UnsafeAccessorKind.Method, Name = "_OnComponentRemoved")]
    private static extern void OnComponentRemoved(NodeComponent nodeComponent, AnimationPlayerComponentHost hostInstance);

    /// <inheritdoc />
    public override Variant _PostProcessKeyValue(Animation @animation, int @track, Variant @value, ulong @objectId, int @objectSubIdx)
    {
        foreach (var component in _components)
        {
            if (component is not AnimationMixerComponent typedComponent) continue;
            return typedComponent._PostProcessKeyValue(@animation, @track, @value, @objectId, @objectSubIdx);
        }
        return base._PostProcessKeyValue(@animation, @track, @value, @objectId, @objectSubIdx);
    }
