...
[Tool, GlobalClass]
public partial class GraphEditComponentHost : GraphEdit, IComponentHost
{
    ...
    private static extern void OnComponentAdded(NodeComponent nodeComponent, GraphEditComponentHost hostInstance);

    [UnsafeAccessor(UnsafeAccessorKind.Method, Name = "_OnComponentRemoved")]
    private static extern void OnComponentRemoved(NodeComponent nodeComponent, GraphEditComponentHost hostInstance);
    ...
    public override Vector2[] _GetConnectionLine(Vector2 @fromPosition, Vector2 @toPosition)
    {
        ...
        {
            if (component is not GraphEditComponent typedComponent) continue;
            return typedComponent._GetConnectionLine(@fromPosition, @toPosition);
        }
    ...
    public override bool _IsInInputHotzone(GodotObject @inNode, int @inPort, Vector2 @mousePosition)
    {
        ...
        {
            if (component is not GraphEditComponent typedComponent) continue;
            return typedComponent._IsInInputHotzone(@inNode, @inPort, @mousePosition);
        }
    ...
    public override bool _IsInOutputHotzone(GodotObject @inNode, int @inPort, Vector2 @mousePosition)
    {
        ...
        {
            if (component is not GraphEditComponent typedComponent) continue;
            return typedComponent._IsInOutputHotzone(@inNode, @inPort, @mousePosition);
        }
    ...
    public override bool _IsNodeHoverValid(StringName @fromNode, int @fromPort, StringName @toNode, int @toPort)
    {
        ...
        {
            if (component is not GraphEditComponent typedComponent) continue;
            return typedComponent._IsNodeHoverValid(@fromNode, @fromPort, @toNode, @toPort);
        }
